  ...
  return (
    <div className="p-6 text-right">
      <h2 className="text-2xl font-bold mb-6 flex flex-row-reverse items-center gap-2">
        <Trophy className="text-indigo-500" /> إدارة الدوريات
      </h2>

      {message && (
        <div className={`p-4 rounded-lg flex flex-row-reverse items-center gap-3 mb-6 ${message.type === 'success' ? 'bg-emerald-900/20 text-emerald-400 border border-emerald-500/20' : 'bg-red-900/20 text-red-400 border border-red-500/20'}`}>
          {message.type === 'success' ? <CheckCircle2 className="w-5 h-5" /> : <Shield className="w-5 h-5" />}
          <p className="font-medium">{message.text === 'League visibility updated' ? 'تم تحديث ظهور الدوري' : message.text === 'League deleted' ? 'تم حذف الدوري' : message.text}</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-slate-800 rounded-lg border border-slate-700 overflow-hidden text-right">
          <div className="p-4 border-b border-slate-700 bg-slate-800 flex flex-row-reverse justify-between items-center">
            <h3 className="font-bold">جميع الدوريات</h3>
            <span className="text-xs font-bold text-slate-400 bg-slate-900 px-2 py-1 rounded">الإجمالي {leagues.length}</span>
          </div>
            ...
            <table className="w-full text-right text-sm">
              <thead className="bg-slate-900/50 text-slate-400 uppercase text-xs">
                <tr>
                  ...
                  <th className="p-4 font-semibold text-right">الكود</th>
                  <th className="p-4 font-semibold text-right">الظهور</th>
              ...
              <tbody className="divide-y divide-slate-700">
                {leagues.map(league => (
                  <tr key={league.id} className={`hover:bg-slate-700/50 transition-colors ${selectedLeague?.id === league.id ? 'bg-indigo-900/20' : ''}`}>
                    <td className="p-4 font-medium flex flex-row-reverse items-center gap-2 justify-start">
                      <Trophy className="w-4 h-4 text-slate-400" /> {league.name}
                    </td>
                    <td className="p-4 font-mono text-indigo-400 text-right">{league.code}</td>
                    <td className="p-4 text-right">
                      <button 
                        onClick={() => togglePublicStatus(league)}
                        className={`flex flex-row-reverse items-center gap-1 text-xs px-2 py-1 rounded font-bold ${league.is_public ? 'bg-emerald-900/40 text-emerald-400' : 'bg-amber-900/40 text-amber-400'}`}
                      >
                        {league.is_public ? <><Unlock className="w-3 h-3" /> عام</> : <><Lock className="w-3 h-3" /> خاص</>}
                      </button>
                    </td>
                    <td className="p-4 text-left space-x-2">
                      <button onClick={() => handleSelectLeague(league)} className="text-slate-400 hover:text-indigo-400 transition-colors ml-2">إدارة</button>
                ...
                {leagues.length === 0 && !loading && (
                  <tr>
                    <td colSpan={4} className="p-8 text-center text-slate-500">لا توجد دوريات حالياً.</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          {selectedLeague ? (
            <div>
              <div className="mb-6">
                ...
                <p className="text-sm text-slate-400 font-mono">الكود: {selectedLeague.code}</p>
              </div>

              <div className="space-y-4 mb-6">
                <div className="bg-slate-900 p-4 rounded-lg flex flex-row-reverse justify-between items-center">
                  <div>
                    <h4 className="font-bold text-sm">إجمالي الأعضاء</h4>
                ...
                <button 
                  ...
                  className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 rounded transition-colors flex flex-row-reverse items-center justify-center gap-2"
                >
                  <Users className="w-4 h-4" /> إضافة جميع مستخدمي التطبيق
                </button>
              </div>

              <div className="border-t border-slate-700 pt-4">
                <h4 className="font-bold text-sm mb-3 text-right">قائمة الأعضاء</h4>
                <div className="max-h-60 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                  {leagueMembers.map(m => (
                    <div key={m.user_id} className="bg-slate-900 p-2 rounded text-sm flex flex-row-reverse items-center gap-2 border border-slate-700">
                      ...
                      <span className="truncate">{m.users?.username || 'مستخدم غير معروف'}</span>
                    </div>
                  ))}
                  {leagueMembers.length === 0 && (
                    <p className="text-xs text-slate-500 text-center py-2">لا يوجد أعضاء بعد.</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-500 py-12">
              ...
              <p>اختر دورياً لإدارة الأعضاء</p>
            </div>
          )}
        </div>
      </div>

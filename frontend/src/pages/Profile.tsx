...
export function Profile() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 md:pb-0">
      <div className="fantasy-card p-0 overflow-hidden border-none shadow-2xl">
        <div className="bg-gradient-to-br from-slate-900 to-indigo-900 text-white px-6 py-10 md:py-16 flex flex-col md:flex-row items-center gap-6 relative overflow-hidden">
          ...
          <div className="text-center md:text-left relative z-10">
            <h1 className="text-3xl md:text-4xl font-black uppercase italic tracking-tighter mb-2">Manager <span className="text-[var(--primary)]">Profile</span></h1>
        ...
        <div className="p-6 md:p-10 bg-gradient-to-b from-[var(--card)] to-[var(--muted)]/30">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="space-y-6">
              <h2 className="text-[10px] font-black text-[var(--muted-foreground)] uppercase tracking-[0.2em] flex items-center gap-2">
                 <UserCircle className="w-4 h-4" /> Personal Details
              </h2>
              <div className="space-y-4">
                <div className="flex items-center gap-4 p-4 bg-[var(--muted)]/50 rounded-2xl border border-[var(--border)] group hover:border-[var(--primary)]/30 transition-all">
                  <div className="p-3 bg-white/5 rounded-xl text-[var(--muted-foreground)]"><Mail className="h-5 w-5" /></div>
                  <div>
                    <p className="text-[8px] font-black text-[var(--muted-foreground)] uppercase tracking-widest">Account Email</p>
                ...
                <div className="flex items-center gap-4 p-4 bg-[var(--muted)]/50 rounded-2xl border border-[var(--border)] group hover:border-[var(--primary)]/30 transition-all">
                  ...
                  <div>
                    <p className="text-[8px] font-black text-[var(--muted-foreground)] uppercase tracking-widest">Username</p>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                 {[
                    ...
                    <button key={i} className="w-full flex items-center justify-between p-4 rounded-2xl bg-[var(--card)] hover:bg-[var(--primary)] hover:text-white transition-all shadow-sm border border-[var(--border)] group">
                 ...
                 <button 
                  ...
                  className="w-full flex items-center gap-4 p-5 rounded-2xl text-rose-500 hover:bg-rose-500 hover:text-white transition-all border border-rose-500/10 hover:border-rose-500 mt-6 shadow-lg shadow-rose-500/5 group"
                >
      ...
      <div className="fantasy-card bg-slate-900 text-white p-6 md:p-8 border-none overflow-hidden relative shadow-2xl">
         <div className="absolute top-0 right-0 w-32 h-32 bg-[var(--primary)]/20 rounded-full blur-3xl -mr-16 -mt-16"></div>
         <div className="flex flex-col md:flex-row justify-between items-center gap-6 relative z-10">
            <div>
               <h3 className="font-black italic uppercase text-xl mb-1 flex items-center gap-2">
                  <HelpCircle className="w-5 h-5 text-[var(--primary)]" /> Community Support
               </h3>
               <p className="text-white/60 text-xs font-bold">Having trouble? Our technical squad is ready to assist you.</p>
            </div>
            <button className="w-full md:w-auto bg-[var(--primary)] text-white font-black py-4 px-10 rounded-xl uppercase text-xs shadow-xl shadow-[var(--primary)]/20 hover:scale-105 transition-all">Submit Ticket</button>
         </div>
      </div>
    </div>
  );

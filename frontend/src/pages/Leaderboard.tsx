  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 md:pb-0">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
         <div>
            <h1 className="text-3xl font-black italic uppercase tracking-tighter mb-2 text-right">الترتيب <span className="text-[var(--primary)]">العام</span></h1>
            <p className="text-[var(--muted-foreground)] font-bold text-right">لوحة الصدارة النهائية لفانتزي لبامة.</p>
         </div>
         <div className="relative group w-full md:w-80">
            ...
            <input 
               type="text" 
               placeholder="ابحث عن مدير فريق..."
               value={searchTerm}
               onChange={(e) => setSearchTerm(e.target.value)}
               className="w-full bg-[var(--card)] border border-[var(--border)] rounded-2xl pr-12 pl-4 py-4 text-sm font-bold focus:ring-2 focus:ring-[var(--primary)] transition-all shadow-sm"
            />
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-10">
         <div className="lg:col-span-2">
            <div className="fantasy-card p-0 overflow-hidden border-none shadow-2xl bg-gradient-to-br from-[var(--card)] to-[var(--muted)]/30">
               <div className="bg-slate-900/5 px-6 py-4 border-b border-[var(--border)] grid grid-cols-12 gap-4 text-[10px] font-black text-[var(--muted-foreground)] uppercase tracking-[0.2em] text-right">
                  <div className="col-span-2">الترتيب</div>
                  <div className="col-span-7">المدير</div>
               ...
               {loading ? (
                  <div className="flex flex-col items-center justify-center py-20 opacity-30 gap-4">
                     ...
                     <p className="font-black uppercase text-xs tracking-widest text-center">جاري حساب الترتيب...</p>
                  </div>
               ) : filteredEntries.length === 0 ? (
                  <div className="py-20 text-center opacity-40">
                     <Trophy className="w-12 h-12 mx-auto mb-4" />
                     <p className="font-black uppercase tracking-widest italic">لم يتم العثور على مدربين</p>
                  </div>
               ) : (
                  <div className="divide-y divide-[var(--border)]">
                     {filteredEntries.map((entry) => (
                        <div key={entry.user_id} className="px-6 py-5 grid grid-cols-12 gap-4 items-center hover:bg-[var(--primary)]/5 transition-all cursor-pointer group">
                           ...
                           <div className="col-span-7 flex items-center gap-4 text-right">
                              ...
                              <div>
                                 <p className="font-black text-[var(--foreground)] truncate uppercase tracking-tight text-sm text-right">{entry.username}</p>
                                 <p className="text-[10px] font-bold text-[var(--muted-foreground)] uppercase text-right">المدرب</p>
                              </div>
                           </div>
         ...
         <div className="space-y-8">
            <section className="fantasy-card p-6 bg-[var(--primary)] text-white border-none shadow-xl relative overflow-hidden group">
               ...
               <h3 className="text-lg font-black italic uppercase tracking-tighter mb-6 flex items-center gap-2 text-right">
                  <Medal className="w-6 h-6 text-amber-400" /> ترتيبي
               </h3>
               <div className="space-y-6 relative z-10 text-right">
                  <div className="flex items-center justify-between">
                     <p className="text-2xl font-black italic">--</p>
                     <p className="text-[10px] font-black uppercase text-white/60 tracking-widest text-right">الترتيب العام</p>
                  </div>
                  <div className="flex items-center justify-between">
                     <p className="text-2xl font-black italic">--</p>
                     <p className="text-[10px] font-black uppercase text-white/60 tracking-widest text-right">النسبة المئوية</p>
                  </div>
                  <button className="w-full py-4 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-xl text-xs font-black uppercase tracking-widest transition-all border border-white/10 flex items-center justify-center gap-2">
                     <ChevronRight className="w-4 h-4 rotate-180" /> عرض التفاصيل 
                  </button>
               </div>
            </section>

            <section className="fantasy-card p-6 space-y-6 text-right">
               <h3 className="text-sm font-black uppercase tracking-widest flex items-center gap-2 justify-end">
                  الأكثر تقدماً <TrendingUp className="w-4 h-4 text-emerald-500" />
               </h3>
               <div className="space-y-4 opacity-50 italic text-xs font-bold text-center">
                  جاري تحديث البيانات...
               </div>
            </section>
         </div>
